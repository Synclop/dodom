...
var processStyles = function(dom,arr,parents){
	if(!arr){
		arr = [];
	}
	var curr;
	var style = (dom.getAllStyles && dom.getAllStyles());
	var classes = (dom.getClasses && dom.getClasses());
	var id = dom.id();
	var tag = dom.tag();
	...
	if(dom._childrenArr){
		var i = 0;
		var l = dom._childrenArr.length
		for(i;i<l;i++){
			curr = dom._childrenArr[i];
			if(curr.render && curr.traverse && curr.style){
		...
		if(arr[i].style){
			...
			if(directive){
				for(var s in arr[i].style){
					...
					ret.push(new Nodes.CSSDirective(directive+pseudo,arr[i].style[s]));
				}
			}
		}
	}
...
var extractStyles = function(dom){
	var arr = processStyles(dom);
